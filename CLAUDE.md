...
- **Chat page convention**: input is `{message, history: [{role, content}]}`, output is `{reply}` (with fallbacks: `message` / `response` / `result` / stringified). Maintained client-side in `frontend/src/app/converse/page.tsx` (Open-WebUI-style: collapsible searchable sidebar, flat full-width assistant messages, hover copy/delete, stop-generation, scroll-to-bottom). The chat page also consumes `trace` WS events and renders agent internals via `AgentTraceInline` as a Cherry-Studio-style collapsible timeline (LLM turns shown as "深度思考", tool/node calls by name with status+duration) placed **above** each assistant answer; on reload it re-hydrates them from the run's `_trace` logs. The tracer (`agentflow/_tracer.py`) is global, so any `get_agent()` / LangGraph run is traced automatically — a plain `run()` that isn't a LangGraph node won't show node steps, but its tool/LLM calls still do.
...
| Chat page agent trace (Cherry-Studio timeline above the answer) | `frontend/src/components/AgentTraceInline.tsx` + `converse/page.tsx` |
