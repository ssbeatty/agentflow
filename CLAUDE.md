...
### Code execution & calculation (built-in calculate / python_exec)

Agents get two more built-in tools alongside `web_search`/`web_fetch` (all four returned by `agentflow/__init__.py::_make_builtin_tools`, so they're on by default for every `get_agent()` / `get_tools()`). Implementation lives in **`backend/agentflow/_sandbox.py`** (stdlib-only, no new deps). Scripts can also call them directly: `from agentflow import safe_calc, run_python`.

- **`calculate` → `safe_calc(expr)`**: a whitelist **AST evaluator** for arithmetic/math. It never spawns a process, imports nothing, and touches no attributes — always safe on untrusted input. Whitelisted operators (`+ - * / // % **`, comparisons, boolean, ternary, list/tuple), constants (`pi`/`e`/`tau`/`inf`), and a curated function set (`sqrt exp log sin cos factorial gcd comb perm hypot abs round min max sum …`). No names/imports/comprehensions/lambdas → `CalcError`. Guards against DoS (`**` exponent capped at 1000, `factorial` arg capped at 1000). This is the "safe interpreter for computation" — use it so the LLM computes instead of hallucinating a number.
- **`python_exec` → `run_python(code)`**: executes arbitrary Python in a **subprocess sandbox**. Isolation is *process*-level (not a Python jail — the child can still `import os`); the guarantees are: (1) **env scrub** — the child env keeps only a small allowlist (`PATH`/`HOME`/`LANG`/`LD_LIBRARY_PATH`/…), so **every `AGENTFLOW_*` var (secrets, LLM keys, OAuth tokens) is dropped** and agent-authored code can't read platform credentials or exfiltrate them; (2) **POSIX rlimits** via `preexec_fn` — `RLIMIT_CPU` (backstop for busy loops), `RLIMIT_AS` (memory, default 1024 MB), `RLIMIT_FSIZE` (default 64 MB), `RLIMIT_CORE=0`, plus `os.setsid()` so a timeout kills the whole process group; (3) **wall-clock `timeout`** (default 15 s) → SIGKILL the group; (4) **isolated temp cwd** (`tempfile.mkdtemp`, `shutil.rmtree`'d after). Runs under `python -I` (isolated mode) using **`sys.executable`** — i.e. the per-script venv python — so packages the user installed (numpy/pandas/…) are importable. A REPL harness prints the final bare expression's repr (notebook-cell style). `allow_network=False` best-effort blocks network via `unshare -rn` where the kernel permits it, silently falling back to allow — **not** a hard guarantee; the real protection is the scrubbed env. Windows degrades gracefully (no `resource`/`preexec_fn`, keeps timeout + cwd + env-scrub, `CREATE_NEW_PROCESS_GROUP`). Non-configurable/global — no DB model or UI; behaviour is code-only in the SDK.
...
| Code exec sandbox + safe calculator (`python_exec` / `calculate` tools, `run_python` / `safe_calc` SDK) | `backend/agentflow/_sandbox.py` (wired into `agentflow/__init__.py::_make_builtin_tools`) |
